...
async def run_job(
    ...
    *,
    ...
    if not claimed:
        return JobResult(
            job_id=job.job_id, run_id=job.run_id, status=JobStatus.succeeded, attempt=job.attempt
        )
    try:
        await handler(job)
        await idempotency.complete(job.idempotency_key, idempotency_ttl_seconds)
        log.info(
            "evaluation_job_succeeded", job_id=job.job_id, run_id=job.run_id, attempt=job.attempt
        )
        return JobResult(
            job_id=job.job_id, run_id=job.run_id, status=JobStatus.succeeded, attempt=job.attempt
        )
        ...
        log.exception(
            "evaluation_job_failed", job_id=job.job_id, run_id=job.run_id, attempt=next_attempt
        )
        return JobResult(
            job_id=job.job_id,
            run_id=job.run_id,
